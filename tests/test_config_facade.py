...
def _load_facade(test_settings_path: str, *, env_prefix: str | None = None):
    ...
    if env_prefix is None:
        return ConfigFacade.load(settings_path=test_settings_path)
    return ConfigFacade.load(settings_path=test_settings_path, env_prefix=env_prefix)


def test_loads_defaults_from_yaml(test_settings_path: str):
    cfg = _load_facade(test_settings_path)
...
def test_env_override_wins(monkeypatch: pytest.MonkeyPatch, test_settings_path: str):
    # Pydantic-style nested env override using double underscore
    monkeypatch.setenv(
        "KQDL__HOSTS__KRX__TRANSPORT__REQUEST_TIMEOUT_SECONDS",
        "7",
    )
    cfg = _load_facade(test_settings_path, env_prefix="KQDL")
    assert cfg.hosts["krx"].transport.request_timeout_seconds == 7


def test_https_enforced_raises(monkeypatch: pytest.MonkeyPatch, test_settings_path: str):
    monkeypatch.setenv("KQDL__HOSTS__KRX__BASE_URL", "http://data.krx.co.kr")
    with pytest.raises(ValueError):
        _ = _load_facade(test_settings_path, env_prefix="KQDL")


def test_retry_policy_loaded(test_settings_path: str):
    cfg = _load_facade(test_settings_path)
...
def test_immutability_of_settings(test_settings_path: str):
    cfg = _load_facade(test_settings_path)
    with pytest.raises(Exception):
        cfg.hosts["krx"].transport.request_timeout_seconds = 99  # type: ignore


def test_idempotent_load(test_settings_path: str):
    a = _load_facade(test_settings_path)
    b = _load_facade(test_settings_path)
